...
\inputleannode{def:quadraticForm}
...
\inputleannode{def:isTrajectoryOn}
...
The predicates below quantify over every solution segment on a compact interval
$[t_0, t_1]$.  This prevents finite escape from making a stability claim vacuous
merely because a non-equilibrium solution is not globally defined.  A solution
segment is an integral curve of the vector field restricted to that interval, in
the sense of \texttt{IsTrajectoryOn}.

\inputleannode{def:lyapunovStable}

\inputleannode{def:locallyExponentiallyStable}

\inputleannode{def:localAsymptoticStable}

\inputleannode{def:globalAsymptoticStable}

\inputleannode{def:unstable}
...
The stable-branch theorem gives uniform local exponential decay on every finite
forward solution segment, without silently excluding solutions that are not
globally defined.

\inputleannode{thm:hurwitz-linearization-locally-exponentially-stable}

\inputleannode{thm:exponential-chetaev-unstable}

\inputleannode{thm:positive-real-eigenvalue-unstable}
